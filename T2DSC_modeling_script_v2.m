...
% Contributor: Nanyan "Rosalie" Zhu, MA in Biotechnology, Columbia University


clc
% Use whole head or whole brain scans
brain_options = {'WB', 'WH'};
pinv_tolerance_list = [8e-7, 8e-8];
...
for brain_idx = 1:2
    clearvars -except brain_idx brain_options pinv_tolerance_list GBMorWT
    close all
    WBorWH = brain_options{brain_idx};

    % glob all the subfolders
    data_path = strcat('./Input/', GBMorWT, '/BySubject/');
    ...
    for subject = 1: length(folder_name_list)
        for pinv_tolerance_idx = 1:2 
            close all
            clearvars -except brain_idx brain_options pinv_tolerance_list GBMorWT folder_name_list ...
                subject pinv_tolerance_idx WBorWH
            % Tune parameters here.
            pinv_tolerance = pinv_tolerance_list(pinv_tolerance_idx);
            coarse_cluster_K = 20;
            refine_cluster_K = 8;
            ...
            % Setup Pathes 
            Input_Path = strcat('./Input/', GBMorWT, '/BySubject/', folder_name_list{subject}, '/');
            Output_Path = strcat('./Output/', GBMorWT, '/BySubject/', folder_name_list{subject}, '/', WBorWH, '/',...
                'pinv_tolerance_', num2str(pinv_tolerance), '_sigma05/');
            ...
            if exist(Output_Path, 'dir')
                ...
                fprintf(strcat(num2str(subject), ' / ', num2str(length(folder_name_list)), ': ', folder_name_list{subject}, ...
                     '   WH or Wb: ', WBorWH, '   Pinv Tolerance: ', num2str(pinv_tolerance)))
                ...
                backgroud_str = load_nifti(strcat(Input_Path,'*median_', WBorWH, '.nii.gz'));
                mask_str = load_nifti(strcat(Input_Path,'*brainmask.nii.gz'));
                DSC4DRaw_str = load_nifti(strcat(Input_Path,'*4d_', WBorWH, '.nii.gz'));
                %%
